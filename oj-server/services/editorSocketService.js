...
module.exports = function(io){ 
	var collaborations = []; // track all the sessions requiring collaboration
	var socketIdToSessionId = {}; // this is a map; hope to get session id through socket id
	...
	io.on('connection', (socket) => {
		...
		socket.on("restoreBuffer", () => {
			let sessionId = socketIdToSessionId[socket.id]; // by giving socket.id, we can find sessionId 
			console.log("restore buffer for sesion: " + sessionId + ", socket: " +
				socket.id);

			if (sessionId in collaborations) {
				let instructions = collaborations[sessionId]['cachedInstructions']; // take all the instructions

				for (let i = 0; i < instructions.length; i++) {
					socket.emit(instructions[i][0], instructions[i][1]); // send back to client, [0] is the event type; [1] is the change content
				}
			} else {
